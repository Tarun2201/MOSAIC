...
class Res_Scoring(nn.Module):
    ...
    def forward(self, input, map_collect, return_map_att=False):
        # input: (b, 3, 224, 224)
        ...
        if return_map_att:
            # For alignment: average over num_classes dimension to get (b, 4, 224, 224)
            map_att_for_alignment = map_att.mean(dim=2)  # (b, 4, 224, 224)
...
# Simple UNet model with 1 layer: one downsampling, then upsampling such that output has same size as input
